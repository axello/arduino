...
//         : V2.00 2019-10-14 - Cleanup into separate files, Axel Roest
//
// **********************************************************************************
...
void updateData(OLEDDisplay *display) {
  ...
  draw.drawProgress(display, 0, "Scanning I2C...");
  NumberOfI2CDevice = i2c_scan();
  // Simulate slow scan to be able to see on display
  for (uint8_t i = 1; i < 100; i++) {
    draw.drawProgress(display, i, "Scanning I2C...");
    delay(2);
  }
  draw.drawProgress(display, 100, "Done...");
...
void setup()
{
  ...
  draw.drawProgress(&display, pbar, F("Scanning WiFi"));
  ...
  pbar = 50;
  draw.drawProgress(&display, pbar, F("Scanning I2C"));
  delay(200);
  NumberOfI2CDevice = i2c_scan();
  Serial.println(F("scan done"));
...
#define WIFI_TIME_OUT 20
  ...
  while ( WiFi.status() != WL_CONNECTED && millis() - this_start < (WIFI_TIME_OUT * 1000) ) {
    ...
    draw.drawProgress(&display, pbar, F("Connecting WiFi"), ssid);
  }
  ...
  draw.drawProgress(&display, 100, F("Setup Done"));
  ...
  ArduinoOTA.onStart([]() {
    ...
    LedRGBOFF();
    Serial.println(F("\r\nOTA Starting"));
    draw.drawProgress(&display, 0, "Starting OTA");
  });
  ...
  ArduinoOTA.onProgress([](unsigned int progress, unsigned int total) {
    ...
    draw.drawProgress(&display, percent, "Uploading", String(buff));
  ...
  ArduinoOTA.onError([](ota_error_t error) {
    LedRGBON(COLOR_RED);
    draw.drawProgress(&display, 0, "Error");
    Serial.println(F("\r\nError"));
  });
  ...
  if (has_display) {
    ...
    ui.setFrames(draw.frames, draw.numberOfFrames);
    ui.init();
    display.flipScreenVertically();
  }
